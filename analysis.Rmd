...
```{r}
# bar chart
ggplot(data = non_british_males, aes(x = ETHNICITY, fill=ETHNICITY)) + 
  geom_bar()
```
...
ggplot(data = subset(non_british_males,!is.na(non_british_males$ethnicity_grouped)), aes(x = ethnicity_grouped, fill=ethnicity_grouped)) + 
  geom_bar()
```
We can explore within each group.
...
p <-ggplot(subset(non_british_males, !is.na(non_british_males$ethnicity_grouped)), aes(x = ETHNICITY, fill=ETHNICITY)) + 
  geom_bar() 

p + facet_grid(cols = vars(ethnicity_grouped))
...
# a. Age at lung cancer diagnosis
```
