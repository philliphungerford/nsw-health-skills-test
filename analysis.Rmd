...
Based on this information, we download the data and check it against the data dictionary. 
```{r data}
##############################################################################
# Prepare data
#=============================================================================
...
```
If we examine the 5 year survival time (1825 days). In the first 5 years, the expected number of days experienced by lung cancer patients is 682 (1.8 years).
...
```
Before sub-setting the data, we can see that the majority of patients are white British. This is expected as the data is simulated from real cancer data in the UK, however may show potential bias towards certain ethnicities when conducting analysis. 
...
```
We can see that most ethnicities are fairly split between genders. With the exception on white and black caribbean's, mixed white and asians, chinese, and others showing slighter higher proportion of females.
...
Here we will examing ethnicity and sex for our lung cancer patients.
...
```
Majority of the patients that were diaagnosed with lung cancer in 2013 were white, followed by asian, blackother and mixed. 
...
ggplot(bysex, aes(x = ethnicity_grouped, y = Percent, fill = SEX)) + 
  geom_bar(position = "stack", stat='identity') + 
  geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 4) + coord_flip() 
```
...
ggplot(bysex, aes(x = ETHNICITY, y = Percent, fill = SEX)) + 
  ...
  geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 4) + coord_flip() 
```
When breaking the ethnicities further, we can see that 80% of males from bangladesh and 70% of males from India were diagnosed with lung cancer.
